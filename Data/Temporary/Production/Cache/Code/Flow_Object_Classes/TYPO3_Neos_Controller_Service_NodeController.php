...
class NodeController extends NodeController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		...
		if (get_class($this) === 'TYPO3\Neos\Controller\Service\NodeController') \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->setInstance('TYPO3\Neos\Controller\Service\NodeController', $this);
		if ('TYPO3\Neos\Controller\Service\NodeController' === get_class($this)) {
	 ...
	 * @return string
	 */
	 public function indexAction($searchTerm, $workspaceName = 'live', array $nodeTypes = array(0 => 'TYPO3.Neos:Document')) {

				// FIXME this can be removed again once Doctrine is fixed (see fixMethodsAndAdvicesArrayForDoctrineProxiesCode())
			$this->Flow_Aop_Proxy_fixMethodsAndAdvicesArrayForDoctrineProxies();
		if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['indexAction'])) {
		$result = parent::indexAction($searchTerm, $workspaceName, $nodeTypes);
			...
			try {
				...
				$methodArguments['nodeTypes'] = $nodeTypes;
					...
					$adviceChain->rewind();
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Service\NodeController', 'indexAction', $methodArguments, $adviceChain);
	 ...
	 */
	 public function showAction($identifier, $workspaceName = 'live') {
		...
		$result = parent::showAction($identifier, $workspaceName);
			...
			try {
				...
				$methodArguments['workspaceName'] = $workspaceName;
					...
					$adviceChain = $adviceChains['TYPO3\Flow\Aop\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Service\NodeController', 'showAction', $methodArguments, $adviceChain);
					$result = $adviceChain->proceed($joinPoint);
	 ...
	 protected function errorAction() {
		...
		$result = parent::errorAction();
			...
			try {
					...
					$adviceChain = $adviceChains['TYPO3\Flow\Aop\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Controller\Service\NodeController', 'errorAction', $methodArguments, $adviceChain);
					$result = $adviceChain->proceed($joinPoint);
	...
	$reflectedClass = new \ReflectionClass('TYPO3\Neos\Controller\Service\NodeController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('TYPO3\Neos\Controller\Service\NodeController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('TYPO3\Neos\Controller\Service\NodeController', $propertyName, 'var');
