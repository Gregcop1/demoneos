...
class TypoScriptService extends TypoScriptService_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	 ...
	 */
	 protected function Flow_Aop_Proxy_buildMethodsAndAdvicesArray() {
		...
		$this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices = array(
			'getMergedTypoScriptObjectTree' => array(
				'TYPO3\Flow\Aop\Advice\AroundAdvice' => array(
					new \TYPO3\Flow\Aop\Advice\AroundAdvice('TYPO3\Neos\Aspects\TypoScriptCachingAspect', 'cacheGetMergedTypoScriptObjectTree', $objectManager, NULL),
				),
			),
		);
	}
	 ...
	 * @throws \TYPO3\Neos\Domain\Exception
	 */
	 public function getMergedTypoScriptObjectTree(\TYPO3\TYPO3CR\Domain\Model\NodeInterface $startNode) {

				// FIXME this can be removed again once Doctrine is fixed (see fixMethodsAndAdvicesArrayForDoctrineProxiesCode())
			$this->Flow_Aop_Proxy_fixMethodsAndAdvicesArrayForDoctrineProxies();
		if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedTypoScriptObjectTree'])) {
		$result = parent::getMergedTypoScriptObjectTree($startNode);

		} else {
			$this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedTypoScriptObjectTree'] = TRUE;
			try {
			
					$methodArguments = array();

				$methodArguments['startNode'] = $startNode;
			
					$adviceChains = $this->Flow_Aop_Proxy_getAdviceChains('getMergedTypoScriptObjectTree');
					$adviceChain = $adviceChains['TYPO3\Flow\Aop\Advice\AroundAdvice'];
					$adviceChain->rewind();
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\Neos\Domain\Service\TypoScriptService', 'getMergedTypoScriptObjectTree', $methodArguments, $adviceChain);
					$result = $adviceChain->proceed($joinPoint);

			} catch (\Exception $e) {
				unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedTypoScriptObjectTree']);
				throw $e;
			}
			unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['getMergedTypoScriptObjectTree']);
		}
