		...
		$result = parent::emitNodePublished($node, $targetWorkspace);
			...
			try {
				...
				$methodArguments['targetWorkspace'] = $targetWorkspace;
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\TYPO3CR\Service\PublishingService', 'emitNodePublished', $joinPoint->getMethodArguments(), NULL, $result);
		...
		$result = parent::emitNodeDiscarded($node);
			...
			try {
				...
				$methodArguments['node'] = $node;
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'TYPO3\TYPO3CR\Service\PublishingService', 'emitNodeDiscarded', $joinPoint->getMethodArguments(), NULL, $result);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
